...
interface NewVisualSettingsDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
function shallowChanged<T>(original: T, next: T) {
...
export function NewVisualSettingsDialog({
  isOpen,
  onClose,
  customFields,
  tags,
  ...
  }, [customFields, isOpen, listDisplay, tags]);
  ...
  const visibleTableFields = useMemo(
    () => orderedFields.filter((field) => field.enabled && isListEligibleCustomField(field) && !isProtectedCustomField(field)),
    [orderedFields]
  );
  ...
  }, [customFields, draftListDisplay, draftTags, listDisplay, orderedFields, tags]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('Error saving new visual settings:', error);
      return false;
    } finally {
      setIsSaving(false);
    }
        ...
        <DialogContent hideCloseButton className="flex h-[min(88vh,960px)] w-[min(96vw,42rem)] max-w-[min(96vw,42rem)] flex-col overflow-hidden gap-0 p-0">
          <DialogHeader className="shrink-0 space-y-0 border-b px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              <DialogTitle className="min-w-0 flex-1 truncate text-base">Configuracoes do Novo Visual</DialogTitle>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-3 pt-2">
            <Tabs defaultValue="table" className="flex min-h-full flex-col gap-4">
              <TabsList className="grid h-auto w-full shrink-0 grid-cols-4 gap-1 rounded-xl border bg-muted/20 p-1">
              ...
              <TabsContent value="table" className="mt-0 space-y-6">
                <div className="space-y-2">
                  <h3 className="text-base font-medium">Conteudo da Tabela</h3>
                  <p className="text-sm text-muted-foreground">
                    Escolha quais tags e campos aparecem nas linhas da tabela de tarefas.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between py-2">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Exibir as tags da atividade na linha da tabela</p>
                    </div>
                    <Switch
                      checked={draftListDisplay.showTags}
                      onCheckedChange={(checked) => setDraftListDisplay((prev) => ({ ...prev, showTags: checked }))}
                    />
                  </div>
                  <div className="flex items-center justify-between py-2">
                    <div>
                      <Label className="font-medium">Mostrar prazo</Label>
                      <p className="text-sm text-muted-foreground">Exibir a data de prazo quando existir</p>
                    </div>
                    <Switch
                      checked={draftListDisplay.showDueDate}
                      onCheckedChange={(checked) => setDraftListDisplay((prev) => ({ ...prev, showDueDate: checked }))}
                    />
                  </div>
                  <div className="flex items-center justify-between py-2">
                    <div>
                      <Label className="font-medium">Mostrar prioridade</Label>
                      <p className="text-sm text-muted-foreground">Exibir a prioridade da atividade na linha</p>
                    </div>
                    <Switch
                      checked={draftListDisplay.showPriority}
                      onCheckedChange={(checked) => setDraftListDisplay((prev) => ({ ...prev, showPriority: checked }))}
                    />
                  </div>
                </div>

                <div className="space-y-3 border-t pt-4">
                  <Label className="text-sm font-medium">Campos visiveis na tabela</Label>
                  {visibleTableFields.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Nenhum campo adicional disponivel para exibir na tabela.</p>
                  ) : (
                    <div className="space-y-2">
                      {visibleTableFields.map((field) => (
                        <div key={field.id} className="flex items-center justify-between py-2">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">Mostrar este campo nas linhas da tabela</p>
                          </div>
                          <Switch
                            checked={(draftListDisplay.visibleFieldIds || []).includes(field.id)}
                            onCheckedChange={(checked) =>
                              setDraftListDisplay((prev) => {
                                const currentVisibleFieldIds = prev.visibleFieldIds || [];
                                const visibleFieldIds = checked
                                  ? [...currentVisibleFieldIds, field.id]
                                  : currentVisibleFieldIds.filter((id) => id !== field.id);

                                return { ...prev, visibleFieldIds };
                              })
                            }
                          />
                        </div>
      ...
      <AlertDialog open={showUnsavedCloseDialog} onOpenChange={setShowUnsavedCloseDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Existem configuracoes alteradas apenas nesse modal. Salve antes de fechar ou descarte para sair sem manter essas mudancas.
            </AlertDialogDescription>
          </AlertDialogHeader>
