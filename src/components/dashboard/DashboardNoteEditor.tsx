...
import { DailyNote, NoteTemplate } from '@/types';
...
function replaceBlockTag(block: HTMLElement, nextTagName: 'p' | 'h1' | 'h2' | 'blockquote') {
  ...
  replacement.append(extractBlockContent(block));
...
function replaceBlockWithBullet(block: HTMLElement) {
  ...
  listItem.append(extractBlockContent(block));
...
function getPreviousEditableBlock(block: HTMLElement) {
  if (block.tagName === 'LI') {
    ...
    if (previousItem instanceof HTMLElement) return previousItem;
...
function renderSaveStatus(saveStatus: SaveStatus, autosaveEnabled: boolean, hasUnsavedChanges: boolean) {
  if (!autosaveEnabled) {
    ...
    if (saveStatus === 'saved') return <span className="text-green-600">Salvo</span>;
    if (saveStatus === 'error') return <span className="text-destructive">Erro ao salvar</span>;
...
export function DashboardNoteEditor({
  currentDate,
  note,
  onReplaceContent,
  onSave,
  ...
  }, [currentDate, hideComments, note.date, noteText, onReplaceContent]);
  ...
  const saveEditorSelection = () => {
    ...
    if (!commonAncestor || !editorRef.current.contains(commonAncestor)) return;
  ...
  const handleToolbarActionMouseDown = (event: React.MouseEvent<HTMLElement>, action: () => void) => {
  ...
  return (
    <div className="flex min-h-[420px] flex-col rounded-[24px] border border-border/60 bg-background shadow-sm">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-4 py-2.5" onMouseDown={handleToolbarMouseDown}>
        <div className="flex flex-wrap items-center gap-0.5 rounded-xl border border-border/60 bg-muted/25 p-1">
          {([
            { id: 'paragraph', icon: Pilcrow, label: 'Texto' },
            { id: 'h1', icon: Heading1, label: 'Titulo' },
            { id: 'h2', icon: Heading2, label: 'Subtitulo' },
            { id: 'bullet', icon: List, label: 'Topico' },
            { id: 'blockquote', icon: Quote, label: 'Citacao' },
            { id: 'comment', icon: MessageSquare, label: 'Comentario' },
            ...
            <Button
              ...
              onMouseDown={(event) => handleToolbarActionMouseDown(event, () => applyBlockStyle(id as RichBlockStyle))}
            >
        ...
        <div className="flex items-center gap-0.5 rounded-xl border border-border/60 bg-muted/25 p-1">
          <Button type="button" variant={isBoldActive ? 'secondary' : 'ghost'} size="icon" className="h-7 w-7 rounded-lg" disabled={!canUseInlineFormatting} onMouseDown={(event) => handleToolbarActionMouseDown(event, () => runEditorCommand('bold'))} title="Negrito (Ctrl+B)">
            <Bold className="h-3.5 w-3.5" />
          </Button>
          <Button type="button" variant={isItalicActive ? 'secondary' : 'ghost'} size="icon" className="h-7 w-7 rounded-lg" disabled={!canUseInlineFormatting} onMouseDown={(event) => handleToolbarActionMouseDown(event, () => runEditorCommand('italic'))} title="Italico (Ctrl+I)">
            <Italic className="h-3.5 w-3.5" />
          </Button>
          <Button type="button" variant={isUnderlineActive ? 'secondary' : 'ghost'} size="icon" className="h-7 w-7 rounded-lg" disabled={!canUseInlineFormatting} onMouseDown={(event) => handleToolbarActionMouseDown(event, () => runEditorCommand('underline'))} title="Sublinhado (Ctrl+U)">
          ...
          <div className="flex items-center gap-0.5">
            <label className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-lg text-muted-foreground hover:bg-accent hover:text-accent-foreground" title="Cor do texto">
            ...
            {PRESET_TEXT_COLORS.map((color) => (
              <button
                ...
                title="Cor do texto"
                onMouseDown={(event) => handleToolbarActionMouseDown(event, () => {
                  setTextColor(color);
          ...
          <div className="flex items-center gap-0.5">
            <label className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-lg text-muted-foreground hover:bg-accent hover:text-accent-foreground" title="Cor de fundo">
            ...
            <Button type="button" variant="ghost" size="icon" className="h-7 w-7 rounded-lg" disabled={!canUseInlineFormatting} onMouseDown={(event) => handleToolbarActionMouseDown(event, () => runEditorCommand('hiliteColor', 'transparent'))} title="Remover cor de fundo">
              <X className="h-3 w-3" />
            </Button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-0.5 rounded-xl border border-border/60 bg-muted/25 p-1">
          {onUndo && (
            <Button type="button" variant="ghost" size="icon" className="h-7 w-7 rounded-lg" disabled={!canUndo} onMouseDown={(event) => handleToolbarActionMouseDown(event, onUndo)} title="Desfazer (Ctrl+Z)">
              <Undo2 className="h-3.5 w-3.5" />
            </Button>
          )}
          {onRedo && (
            <Button type="button" variant="ghost" size="icon" className="h-7 w-7 rounded-lg" disabled={!canRedo} onMouseDown={(event) => handleToolbarActionMouseDown(event, onRedo)} title="Refazer (Ctrl+Y)">
          ...
          <Popover>
            ...
            <PopoverContent align="end" className="w-72 text-xs">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-muted-foreground">
                <span><kbd className="font-medium text-foreground">Ctrl+1</kbd> Titulo</span>
                <span><kbd className="font-medium text-foreground">Ctrl+2</kbd> Subtitulo</span>
                <span><kbd className="font-medium text-foreground">Ctrl+3</kbd> Citacao</span>
                <span><kbd className="font-medium text-foreground">Ctrl+4</kbd> Topico</span>
                <span><kbd className="font-medium text-foreground">Ctrl+5</kbd> Comentario</span>
                ...
                <span><kbd className="font-medium text-foreground">Ctrl+I</kbd> Italico</span>
                ...
                <span><kbd className="font-medium text-foreground"># </kbd> → Titulo</span>
                <span><kbd className="font-medium text-foreground">## </kbd> → Subtitulo</span>
                <span><kbd className="font-medium text-foreground">- </kbd> → Topico</span>
                <span><kbd className="font-medium text-foreground">&gt; </kbd> → Citacao</span>
                <span><kbd className="font-medium text-foreground">// </kbd> → Comentario</span>
              </div>
            </PopoverContent>
          </Popover>
        </div>
      ...
      <div className="flex-1 px-4 py-4">
        <div
          ...
          className="rich-note-editor min-h-[260px] w-full rounded-2xl border border-transparent bg-transparent px-2 text-[15px] leading-6 text-foreground outline-none [&_[data-collapse-toggle]]:no-underline [&_[data-collapse-toggle]]:not-italic [&_blockquote]:my-1 [&_blockquote]:ml-0 [&_blockquote]:border-l-4 [&_blockquote]:border-border [&_blockquote]:pl-4 [&_blockquote]:italic [&_h1]:mb-1 [&_h1]:mt-2 [&_h1]:ml-0 [&_h1]:px-0 [&_h1]:py-0 [&_h1]:text-[2rem] [&_h1]:font-bold [&_h1]:leading-tight [&_h2]:mb-1 [&_h2]:mt-2 [&_h2]:ml-0 [&_h2]:px-0 [&_h2]:py-0 [&_h2]:text-[1.5rem] [&_h2]:font-semibold [&_h2]:leading-tight [&_h1[data-collapsible-heading='true']]:relative [&_h2[data-collapsible-heading='true']]:relative [&_ol]:my-1 [&_ol]:ml-0 [&_ol]:pl-8 [&_p]:my-1 [&_p]:ml-0[&_ul]:my-1 [&_ul]:ml-0 [&_ul]:list-disc [&_ul]:pl-8"
